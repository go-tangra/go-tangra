...
- **Dev stack** — optional `openldap` service under the compose profile `ldap`
  in `deploy/stack/compose.yaml` (T067) builds this copy (`deploy/stack/ldap/openldap`).
...
its CA under `deploy/stack/ldap/` (T067). Both listeners verify against the
...
```sh
# 1. Build
docker build -t go-tangra-openldap-test deploy/stack/ldap/openldap
