...
# deploy/stack/ldap/ca.pem (mounted at /out) — that is the PEM pasted into a
...
# Hand the CA to the host, owned by whoever owns deploy/stack/ldap.
cp ca.crt /out/ca.pem
chown "$(stat -c %u:%g /out)" /out/ca.pem
chmod 0644 /out/ca.pem
echo "wrote deploy/stack/ldap/ca.pem"
