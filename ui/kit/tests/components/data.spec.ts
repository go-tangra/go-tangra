...
describe('UiDataTable', () => {
  ...
  it('rowSelectable disables individual rows; select all only toggles selectable rows', async () => {
    setViewport(1280)
    const rowSelectable = (r: Record<string, unknown>) => r.id !== '2'
    const w = mountBody(UiDataTable, { props: { items: rows, columns, selectable: true, selected: ['2'], rowSelectable } })
    const boxes = w.findAll('tbody input[type=checkbox]')
    expect(boxes.map((b) => (b.element as HTMLInputElement).disabled)).toEqual([false, true, false])
    ...
    // the locked row keeps its state; only the selectable rows are added
    expect(w.emitted('update:selected')?.[0]).toEqual([['2', '1', '3']])
    await w.setProps({ selected: ['2', '1', '3'] })
    expect((all.element as HTMLInputElement).checked).toBe(true)
    await all.trigger('change')
    expect(w.emitted('update:selected')?.[1]).toEqual([['2']])
    await w.setProps({ selected: [] })
    await all.trigger('change')
    expect(w.emitted('update:selected')?.[2]).toEqual([['1', '3']])
    await w.setProps({ rowSelectable: () => false })
    expect((w.find('thead input[type=checkbox]').element as HTMLInputElement).disabled).toBe(true)
    await expectA11y(w.element)
    w.unmount()
    setViewport(360)
    const s = mountBody(UiDataTable, { props: { items: rows, columns, selectable: true, selected: [], rowSelectable } })
    const cards = s.findAll('li input[type=checkbox]')
    expect(cards.map((b) => (b.element as HTMLInputElement).disabled)).toEqual([false, true, false])
    await cards[1]!.trigger('click')
    expect(s.emitted('update:selected')).toBeUndefined()
    await cards[0]!.trigger('click')
    expect(s.emitted('update:selected')?.[0]).toEqual([['1']])
    s.unmount()
  })
